...
export default function About() {
  ...
  return (
    <Section id="about" eyebrow="about" title="Builder at heart.">
      <div className="grid gap-10 lg:grid-cols-[1.6fr_1fr]">
        ...
        {/* "Currently" IDE card + photo stacked */}
        <div className="flex flex-col gap-6">
          ...
          <div className="group relative overflow-hidden rounded-xl border border-line">
            <img
              src={picture}
              alt="Thien-An Tran"
              className="aspect-[4/3.6] w-full object-cover object-[center_18%] transition-transform duration-500 group-hover:scale-105"
            />
            <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-bg/95 via-bg/50 to-transparent p-4 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
              <p className="mb-2.5 font-mono text-[10px] uppercase tracking-widest text-accent">// off the clock</p>
              <div className="flex flex-wrap gap-1.5">
                {['🎹 Piano', '🎸 Guitar', '🧗 Climbing', '⛷️ Skiing'].map(hobby => (
                  <span
                    key={hobby}
                    className="rounded-full border border-line bg-surface/80 px-3 py-1 font-mono text-xs text-ink backdrop-blur-sm"
                  >
                    {hobby}
                  </span>
                ))}
